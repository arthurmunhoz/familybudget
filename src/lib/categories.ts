...
/** A household's custom NAME for a built-in preset, or null to use the localized
 *  default. Callers render built-in names via t(`cat.<id>`), so they need to know
 *  when an override should win over the translation. */
export function overriddenName(id: string, overrides?: CategoryOverride[]): string | null {
  return overrides?.find((o) => o.base_id === id)?.name ?? null
}
