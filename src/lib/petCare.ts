...
// Kept in sync with mobile/src/lib/petCare.ts — same table contract, same math.
...
/** A pet's daily checklist in order, with that day's completion (if any). */
export function dailyChecklist(
  ...
  return tasks
    ...
    .map((task) => ({
      task,
      done: done.find((d) => d.task_id === task.id && d.done_on === day) ?? null,
    }))
}
...
export function templateTasks(
  ...
  if (species === 'cat') {
    return [
      ...base.slice(0, 2),
      { key: 'litter', icon: 'paw', kind: 'daily', interval_days: null },
      ...base.slice(2),
    ]
  }
  return base
