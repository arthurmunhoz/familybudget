...
| **Calendar** | ✅ month grid + Upcoming, recurrence, color-by-member, kind markers + "turns N", add/edit, **Google Calendar connect/sync** (in-app OAuth, reuses deployed API) | Google connect needs the calendar scope on your Google OAuth consent screen + on-device test — see §3.E |
