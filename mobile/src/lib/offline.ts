// Offline support for the shopping list — the native port of the PWA's
// src/lib/offline.ts. AsyncStorage replaces localStorage, so the durable
// helpers are async here. The outbox queues mutations made with no connection
// (the "no wifi at Costco" case) and replays them, in order, on reconnect.
import AsyncStorage from '@react-native-async-storage/async-storage'

import { supabase } from './supabase'
import type { ShoppingItem, ShoppingStore } from './types'

/** Durable JSON storage — survives app restarts, unlike the in-memory query
...
export const LOCAL_ITEMS = 'shopping:items'
export const LOCAL_STORES = 'shopping:stores'
...
export async function enqueueOp(op: ShoppingOp): Promise<void> {
  const q = (await loadLocal<ShoppingOp[]>(OUTBOX_KEY)) ?? []
  q.push(op)
  await saveLocal(OUTBOX_KEY, q)
}
...
export async function flushShoppingOutbox(): Promise<void> {
  if (flushing) return
  let q = (await loadLocal<ShoppingOp[]>(OUTBOX_KEY)) ?? []
  if (!q.length) return
  flushing = true
  try {
    while (q.length) {
...
/** Type re-exports so the screen can persist typed snapshots. */
export type { ShoppingItem, ShoppingStore }
