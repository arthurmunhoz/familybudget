// Add/edit a pet — a full-screen modal. Mirrors the PWA's PetForm: name, emoji,
// species, breed, birthday, colors, weight, length, microchip, notes, and an
// optional photo (picked from the library, resized, uploaded to the private
// `documents` bucket under `<household_id>/pets/<uuid>.jpg`).
...
// PetForm owns the birthday label (it carries an "(optional)" suffix).
function BirthdayPicker({ value, onChange }: { value: string; onChange: (iso: string) => void }) {
  const { t } = useI18n()
  return (
    <DateField
